...
PkxDecoder.prototype.decode = function(buffer, callback) {
    ...
    async.parallel({
        ...
        met_location: function(cb) {
            veekun.location(pokemon.origin.met_location, gen, cb)
        },
        egg_location: function(cb) {
            veekun.location(pokemon.origin.egg_location, gen, cb)
        },
        pokeball: function(cb) {
            veekun.pokeball(pokemon.origin.pokeball, cb)
        }
    }, function(err, results) {
        if (err) {
            console.log("datanne sensorille error on:", err)
            emit('error', err)
        } else {
            ...
            pokemon.origin.met_location_name = results.met_location
            pokemon.origin.egg_location_name = results.egg_location
            pokemon.origin.pokeball_name = results.pokeball
